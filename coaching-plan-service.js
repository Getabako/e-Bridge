...
${planStructure.totalWeeks}週分のプランを生成してください。各週は上記の例のように、Valorantの実践的で測定可能な内容にしてください。`;
    ...
    getAllPlans() {
        try {
            const plans = JSON.parse(localStorage.getItem('coaching_plans') || '[]');

            // 不正なプランデータを修復
            let needsSave = false;
            const repairedPlans = plans.map(plan => {
                // weeks が配列でない場合（二重ラップされている場合）を修復
                if (plan.weeks && !Array.isArray(plan.weeks)) {
                    console.warn(`🔧 修復中: プラン ${plan.id} の weeks が不正な構造です`);

                    // plan.weeks が plan オブジェクトになっている場合
                    if (plan.weeks.weeks && Array.isArray(plan.weeks.weeks)) {
                        plan.weeks = plan.weeks.weeks;
                        needsSave = true;
                        console.log(`✅ プラン ${plan.id} を修復しました`);
                    } else {
                        // 修復不可能な場合は削除対象としてマーク
                        console.error(`❌ プラン ${plan.id} は修復不可能です。削除します。`);
                        return null;
                    }
                }
            ...
            // 修復が必要だった場合は保存
            if (needsSave || repairedPlans.length !== plans.length) {
                localStorage.setItem('coaching_plans', JSON.stringify(repairedPlans));
                console.log(`📝 ${repairedPlans.length}件のプランを保存しました`);
            }
